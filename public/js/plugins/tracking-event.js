...
var BufferTracking = function BufferTracking(config) {
  ...
  // Xử lý trường hợp Video buffer quá lâu
  var infiniteBufferingFlag = false;
  var infiniteTimer = null;
  var endInfiniteBuffering = false;
  ...
  var onPause = function onPause() {
    ...
    infiniteBufferingFlag = false;
    endInfiniteBuffering = false;
  };
  ...
  var isInfiniteBuffering = function isInfiniteBuffering() {
    ...
    _this.trigger('tracking:buffered', {
      currentTime: +curTime,
      readyState: +readyState,
      secondsToLoad: +10, //+secondsToLoad.toFixed(3),
      bufferCount: +bufferCount,
    });
  ...
  if (infiniteBufferingFlag && !endInfiniteBuffering && checkOnTimeupdate) {
    isInfiniteBuffering();
  }
};
